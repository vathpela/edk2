  ...
  )
{
  ...
  if (Private->GraphicsOutput != NULL) {
    ...
    if ((HorizontalResolution == 0x0) || (VerticalResolution == 0x0)) {
      ...
      if (EFI_ERROR (Status)) {
        ...
        if (Mode->MaxMode != 0) {
          //
          // Get the current mode information either Set default mode failed or Successed
          //
  ...
  //
  Status = GraphicsConsoleConOutSetMode (&Private->SimpleTextOutput, 2);
  if (EFI_ERROR (Status)) {
    goto Error;
  }

  // Remove for boot time optimization
  /*
  DEBUG_CODE_BEGIN ();
